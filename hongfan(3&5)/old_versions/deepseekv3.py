...
async def handle_user_choice(user_id, session_id, choice, chat_history):
    ...
    if choice == "remove":
        ...
        new_history = [
            ...
            if not (msg["content"] == pending["content"] or 
                   ("⚠️" in msg.get("content", "") and msg["role"] == "system"))
        ]
        ...
        new_history = [
            msg for msg in storage_history 
            if not ("⚠️" in msg.get("content", "") and msg["role"] == "system")
        ]
...
with gr.Blocks() as demo:
    ...
    def toggle_action_panel(history):
        """修复空历史记录导致的类型错误"""
        try:
            if history and history[-1][1]:  # 检查最后一条消息的bot部分
